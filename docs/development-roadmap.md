...
大部分设置项已经是输入框，用户需要手动输入仓库名、分支名、路径等。部分字段（如 `layout`）只有几个固定选项却用文本框。

**方案**

- **GitHub 仓库**：填入 token + owner 后，调用 GitHub API 列出该用户/组织的仓库列表，用下拉选择替代手动输入
- **分支**：选中仓库后，列出分支列表供选择
- **Layout**：改为下拉选择（post / draft / page），已有选项值
- **Categories**：已有分类时展示为 chip 列表，点击添加；保留"其他"选项允许手动输入新分类
...
| 3 | 4. 减少手动输入（第一批） | 先做 layout/categories/tags 的选择器，纯本地改动 |
